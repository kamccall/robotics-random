...
private:
    ...
    // maptogrid function to process log values in map and convert to 0 || 1 in grid
    vector<vector<int>> MaptoGrid()
    {
        vector<vector<int>> grid(mapHeight, vector<int>(mapWidth));
        // map values: 0:unknown, <0:free, >0:occupied
        // 0 <-- free space, 1 <-- occupied and unknown 
        for (int i = 0; i < mapHeight; i++)
        {
            for (int j = 0; j < mapWidth; j++)
            {
    ...
    // heuristic vector using manhatten distance
...
{
public:
    int start[2] = { 230, 145 };  // start location in search grid
    int goal[2]  = { 60, 50 };    // goal location in search grid
    int cost = 1;
